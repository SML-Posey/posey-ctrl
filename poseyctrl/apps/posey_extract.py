...
def posey_extract():
        ...
        if mid == fbm.message.message_id:
            ...
            if copy_size > 0:
                copy_to_flash_buffer(
                    slot, np.frombuffer(fb_listener.read(copy_size), "u1")
                )
                block_bytes -= copy_size
            if block_bytes > 0:
                copy_to_flash_buffer(
                    slot, np.frombuffer(data[cursor : cursor + block_bytes], "u1")
                )
                cursor += block_bytes
    ...
    for slot in flash_data_buffers:
        ...
        empty_queue(pq)
        prefix = (
            os.path.basename(args.filename).replace(".npz", "") if args.prefix else ""
        )
        ...
        log.info(
            "Extracting flash data for slot %d (%.2f MB)...",
            slot,
            flash_cursor / 1024.0 / 1024.0,
        )
        flash_data = flash_data[:flash_cursor].tobytes()
        sensor = hil.PoseyHIL(prefix, qout, qin, pq, None, None, None, output_raw=None)
